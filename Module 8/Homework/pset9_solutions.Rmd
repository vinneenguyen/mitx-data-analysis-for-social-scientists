...
lwage_se.linear <- lm(lwage ~ yrs_school + ttl_exp, data = women)
summary_lwage_se <- summary(lwage_se.linear)
summary_lwage_se

"R-squared"
summary_lwage_se$r.squared
```
...
```{r}
"c, f"
"Possible restricted models can be obtained by plugging null hypothesis (2(Beta 1) = (Beta 2)) into original model"
```
...
```{r}
"Data for restricted model under null 2(Beta 1) = (Beta 2)"
women_restricted <- read.csv(data_file)
women
summary(women)
women_
```
